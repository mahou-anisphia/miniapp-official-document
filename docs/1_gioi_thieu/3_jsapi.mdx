...
## jsAPI cho phép bạn làm gì?

jsAPI là cầu nối giữa miniapp và các tính năng native của thiết bị. Nhờ đó, miniapp có thể:
...
:::tip Web app thuần có thể chạy mà không cần jsAPI
jsAPI là **tùy chọn**. Nếu miniapp chỉ cần logic nghiệp vụ cơ bản và không cần truy cập phần cứng, bạn hoàn toàn có thể bỏ qua phần này.
:::
...
## Giới hạn kỹ thuật của jsAPI

jsAPI mạnh mẽ nhưng **không phải là native app**. Một số giới hạn cần lưu ý:
...
**Luồng:** Miniapp → jsAPI → **SDK đã tích hợp sẵn trong Superapp** → Thiết bị
...
**Khi nào cần:** Khi SDK yêu cầu quyền truy cập native (ví dụ: SDK xác thực sinh trắc học, SDK VoIP) mà jsAPI hiện tại không hỗ trợ.
...
Repository mẫu demo **toàn bộ jsAPI** có sẵn:
...
**Chưa chắc cần jsAPI?** Xem [Lựa chọn phương thức tích hợp](./lua_chon_tich_hop) để so sánh các phương án phù hợp với nhu cầu của bạn.

**Đã quyết định dùng jsAPI?** Tiếp tục với:
